...
module turbos_clmm::compute_swap_step_tests {
	...
	public fun compute_swap_max_liquidity_tests() {
		...
		let amount = i128::from(1000000000000);
		let fee = 10000;
		let (sqrt_price, step_amount_in, step_amount_out, step_fee_amount)  = math_swap::compute_swap(price, price_target, liquidity, amount, fee);
